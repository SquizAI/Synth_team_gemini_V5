...
export default function App() {
  ...
  const uploadVideo = async (e) => {
    ...
    if (!validTypes.includes(videoFile.type)) {
      ...
      setProcessingError(
        'Invalid file type. Please upload an MP4, WebM, MOV, or QuickTime video.'
      );
      setIsLoadingVideo(false);
      return;
    }
    ...
    try {
      ...
      const resp = await fetch(`/api/upload/complete/${fileId}`).then((r) =>
        r.json()
      );
      if (resp.error) {
      ...
      localStorage.setItem(
        'uploadedVideos',
        JSON.stringify([...existingVideos, videoInfo])
      );
    ...
    <Layout>
      ...
      >
        ...
        <div className={c('tools', { inactive: !vidUrl })}>
          ...
          {showCollaboration && <Collaboration videoFile={file} />}

          {showProcessing && <AdvancedProcessing videoFile={file} />}
